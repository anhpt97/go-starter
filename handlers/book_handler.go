...
func (handler *BookHandler) Get(w http.ResponseWriter, r *http.Request) {
	book, err := handler.bookRepository.FindOne(w, r, bson.M{"_id": utils.ConvertStringToObjectID(mux.Vars(r)["id"])})
	if err != nil {
		return
	}
...
func (handler *BookHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	if dto.UserID != nil {
		_, err := handler.userRepository.FindOne(w, r, bson.M{"_id": dto.UserID})
		if err != nil {
			return
		}
	}

	book, err := handler.bookRepository.Create(w, r, data)
	if err != nil {
		return
	}
...
func (handler *BookHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	book, err := handler.bookRepository.Update(w, r, utils.ConvertStringToObjectID(mux.Vars(r)["id"]), data)
	if err != nil {
		return
	}
...
func (handler *BookHandler) Delete(w http.ResponseWriter, r *http.Request) {
	err := handler.bookRepository.Delete(w, r, utils.ConvertStringToObjectID(mux.Vars(r)["id"]))
	if err != nil {
		return
	}
